...
<a class="navbar-brand" id="logo">新聞列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="news-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>nwsID</th>
                    <th>nwsType</th>
                    <th>nwsName</th>
                    <th>nwsPic</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/news-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("新聞列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].nwsID+'</td><td>'+data[i].nwsType+'</td><td>'+data[i].nwsName+'</td><td>'+data[i].nwsPic+'</td><td>'+data[i].nwsInfo+'</td><td><a href="news-update.php?ID='+data[i].nwsID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].nwsID+'" href="news-delete-api.php?ID='+data[i].nwsID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
          $("#news_list").append(strHTML);
        }
      }
      ...
      function del_item(myevent){
        if(confirm("確定刪除資料"+$(myevent).data("id")+"??")){
          $.ajax({
            type: "POST",
            url: "http://acgassistant2019.000webhostapp.com/BackEnd/news-delete-api.php",
