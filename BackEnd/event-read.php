...
<a class="navbar-brand" id="logo">活動事件列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="event-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>eveID</th>
                    <th>eveType</th>
                    <th>eveSubType</th>
                    <th>eveLocationType</th>
                    <th>eveName</th>
                    <th>evePic</th>
                    <th>eveStartDate</th>
                    <th>eveEndDate</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/event-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("會員列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].eveID+'</td><td>'+data[i].eveType+'</td><td>'+data[i].eveSubType+'</td><td>'+data[i].eveLocationType+'</td><td>'+data[i].eveName+'</td><td>'+data[i].evePic+'</td><td>'+data[i].eveStartDate+'</td><td>'+data[i].eveEndDate+'</td><td>'+data[i].evelocation+'</td><td>'+data[i].eveWeb+'</td><td>'+data[i].eveMap+'</td><td>'+data[i].eveInfo+'</td><td><a href="event-update.php?ID='+data[i].eveID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].eveID+'" href="event-delete-api.php?ID='+data[i].eveID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
          $("#event_list").append(strHTML);
        }
      }
