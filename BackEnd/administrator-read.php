...
<a class="navbar-brand" id="logo">管理者列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="administrator-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>admID</th>
                    <th>admName</th>
                    <th>admPwd</th>
                    <th>編輯</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/administrator-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("管理者列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].admID+'</td><td>'+data[i].admName+'</td><td>'+data[i].admPwd+'</td><td><a href="administrator-update.php?ID='+data[i].admID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].admID+'" href="administrator-delete-api.php?ID='+data[i].admID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
          $("#administrator_list").append(strHTML);
        }
      }
      ...
      function del_item(myevent){
        if(confirm("確定刪除資料"+$(myevent).data("id")+"??")){
          $.ajax({
            type: "POST",
            url: "http://acgassistant2019.000webhostapp.com/BackEnd/administrator-delete-api.php",
