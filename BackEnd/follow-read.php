...
<a class="navbar-brand" id="logo">跟隨列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="follow-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>floID</th>
                    <th>floMemID</th>
                    <th>floItemID</th>
                    <th>floType</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/follow-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("會員列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].floID+'</td><td>'+data[i].floMemID+'</td><td>'+data[i].floItemID+'</td><td>'+data[i].floType+'</td><td><a href="follow-update.php?ID='+data[i].floID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].floID+'" href="follow-delete-api.php?ID='+data[i].floID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
          $("#follow_list").append(strHTML);
        }
      }
      ...
      function del_item(myevent){
        if(confirm("確定刪除資料"+$(myevent).data("id")+"??")){
          $.ajax({
            type: "POST",
            url: "http://acgassistant2019.000webhostapp.com/BackEnd/follow-delete-api.php",
