...
<a class="navbar-brand" id="logo">動畫列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="animation-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>aniID</th>
                    <th>aniType</th>
                    <th>aniName</th>
                    <th>aniPic</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/animation-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("會員列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].aniID+'</td><td>'+data[i].aniType+'</td><td>'+data[i].aniName+'</td><td>'+data[i].aniPic+'</td><td>'+data[i].aniStartDate+'</td><td>'+data[i].aniEndDate+'</td><td>'+data[i].aniUplaodDay+'</td><td>'+data[i].aniTrailer+'</td><td>'+data[i].aniInfo+'</td><td><a href="animation-update.php?ID='+data[i].aniID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].aniID+'" href="animation-delete-api.php?ID='+data[i].aniID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
          $("#animation_list").append(strHTML);
        }
      }
