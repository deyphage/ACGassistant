...
<a class="navbar-brand" id="logo">評論列表</a>
  ...
  <div class="row">
        ...
        <div class="row">
          <div class="col-sm-10 col-sm-offset-1">
              <table class="table mt10">
                <thead>
                  <tr>
                    <tr><a href="comment-create.php" class="btn btn-sm btn-success mr5">新增</a></tr>
                    <th>cmtID</th>
                    <th>cmtMemID</th>
                    <th>cmtNews</th>
                    <th>cmtInfo</th>
	...
	<script>
    	$(function(){
    		$.ajax({
    			type: "GET",
    			url: "http://acgassistant2019.000webhostapp.com/BackEnd/comment-read-api.php",
    			dataType: "json",
    			success: showMember,
    			error:function(){
						alert("會員列表api回傳失敗");
        ...
        for(i=0; i<data.length; i++){
          strHTML = "";
          strHTML = '<tr><td>'+data[i].cmtID+'</td><td>'+data[i].cmtMemID+'</td><td>'+data[i].cmtNews+'</td><td>'+data[i].cmtInfo+'</td><td><a href="comment-update.php?ID='+data[i].cmtID+'" class="btn btn-sm btn-primary mr5">更新</a><a data-id="'+data[i].cmtID+'" href="comment-delete-api.php?ID='+data[i].cmtID+'" class="btn btn-sm btn-danger" onclick="del_item(this)">刪除</a></td></tr>';
